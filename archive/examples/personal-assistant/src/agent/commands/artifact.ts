...
export function artifactUpdate(projectPath: string, artifactId: string, patch: UpdateArtifactInput): ArtifactUpdateResult {
  if (patch.content && patch.content.length > AGENT_MD_MAX_CHARS) {
    const resolved = findArtifactById(projectPath, artifactId)?.artifact?.id
    if (artifactId === AGENT_MD_ID || resolved === AGENT_MD_ID) {
