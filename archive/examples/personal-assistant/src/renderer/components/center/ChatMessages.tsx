  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} group`}>
      <div
        ...
        }${!isUser && isSaved ? ' border-l-2' : ''}`}
        style={{
          background: isUser
            ? 'var(--color-bubble-user)'
            : 'var(--color-bubble-assistant)',
          ...(!isUser && isSaved ? { borderColor: 'var(--color-status-success)' } : {})
        }}
        data-msg-id={msg.id}
      >
...
function ThinkingDots() {
  return (
    <span className="inline-flex gap-1 items-center">
      <span className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ animationDelay: '0ms', background: 'var(--color-accent-soft)' }} />
      <span className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ animationDelay: '150ms', background: 'var(--color-accent-soft)' }} />
      <span className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ animationDelay: '300ms', background: 'var(--color-accent-soft)' }} />
    </span>
  )
}
  ...
  return (
    <div className="flex justify-start">
      ...
      >
        ...
        <span className="inline-block w-1.5 h-4 animate-pulse ml-0.5 align-text-bottom" style={{ background: 'var(--color-accent-soft)' }} />
      </div>
    </div>
  )
}
...
export function ChatMessages() {
  ...
  useEffect(() => {
    ...
    if (el) {
      ...
      setTimeout(() => el.classList.remove('ring-2', 'ring-[var(--color-accent)]'), 1500)
    }
  }, [scrollToMessageId])
