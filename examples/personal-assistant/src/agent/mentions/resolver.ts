...
async function resolveOne(ref: MentionRef, projectPath: string): Promise<ResolvedMention> {
  try {
    switch (ref.type) {
      ...
      case 'doc':
        return resolveEntity(ref, join(projectPath, PATHS.docs), 'doc')
      case 'file':
 ...
 * Matches by: exact id, id prefix, title substring.
 */
...
function entityName(entity: Entity): string {
  ...
  if (entity.type === 'doc') return (entity as Doc).title
  return entity.id
}
...
function formatEntityContent(entity: Entity): string {
  ...
  if (entity.type === 'doc') {
    ...
    return `Title: ${doc.title}\nFile: ${doc.filePath}${mime}${desc}${content}`
  }
