...
usage() {
  cat <<'HELP'
usage: profile-delete.sh --profile-id <id> [options]
...
if ! clab_api_request DELETE "/profiles/${PROFILE_ID}"; then
  if [[ "$CLAB_LAST_HTTP_CODE" == "404" && "$ALLOW_MISSING" == "true" ]]; then
    DELETE_STATE="already_missing"
  else
    fail "profile delete failed: http=$CLAB_LAST_HTTP_CODE $(clab_api_error_message)" 3
  fi
fi
