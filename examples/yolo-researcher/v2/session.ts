...
export class YoloSession {
  ...
  private async applyNativeTurnProjectMutations(input: NativeTurnProjectMutationInput): Promise<NativeTurnProjectMutationResult> {
    ...
    const updateSummaryLines = [
      ...
      ...input.outcomeUpdateSummary.map((line) => line.trim()).filter(Boolean)
    ]
    ...
    let projectUpdated = await this.applyOutcomeProjectUpdate({
      projectUpdate: input.outcomeProjectUpdate,
      plannerCheckpointDue: input.plannerCheckpointDue,
      evidenceRefMap: input.evidenceRefMap,
      fallbackEvidencePath: this.toEvidencePath(input.resultPath),
      toolEvents: input.toolEvents,
      artifactsDir: input.artifactsDir,
      updateSummaryLines
    })
    ...
    const planDeltaResult = await this.applyNativeTurnPlanDeltas({
      activePlanId: input.activePlanId,
      statusChange: input.statusChange,
    ...
    const doneEntries = (
      finalStatus === 'success' && input.deltaReasons.length > 0 && input.actionFingerprint
        ? [{
          ...
          evidencePath: input.businessArtifactEvidencePaths[0] ?? this.toEvidencePath(input.resultPath)
        }]
        : []
    )
    ...
    if (finalStatus === 'success' && input.deltaReasons.length > 0 && input.actionFingerprint) {
      clearedRedundancyBlocked = await this.failureStore.clearRedundancyBlocked({
        ...
        evidencePath: this.toEvidencePath(input.resultPath)
      })
    }
  ...
  private async buildNativeTurnResultPayload(input: {
    ...
    const claimsCoverage = input.persistedProject.claims.length > 0
      ? {
        ...
        claims_covered: input.persistedProject.claims.filter(c => c.status === 'covered').length,
        claims_coverage: Number((input.persistedProject.claims.filter(c => c.status === 'covered').length / input.persistedProject.claims.length).toFixed(2))
      }
      : {}
    const goalConstraintsFingerprint = this.computeGoalConstraintsFingerprint(input.persistedProject)
    const planBoardHash = this.computePlanBoardFingerprint(input.persistedProject)
    ...
    return {
      ...
      resultPayload: {
        ...
        tool_events_path: this.toEvidencePath(input.toolEventsPath),
  ...
  private prepareNativeTurnPlanProgress(input: NativeTurnPlanProgressInput): NativeTurnPlanProgressResult {
    ...
    const projectedPlanIds = new Set<string>(input.currentBoard.map((item) => item.id))
    ...
    let statusChange = ''
    let doneDefinitionCheck: PlanDoneDefinitionCheck = {
      ok: true,
      reason: '',
  ...
  private async applyNativeTurnStatusGuards(input: NativeTurnStatusGuardInput): Promise<NativeTurnStatusGuardResult> {
    ...
    let doneDefinitionCheck: PlanDoneDefinitionCheck = { ...input.doneDefinitionCheck }
    ...
    const onlyRunArtifactsTouched = input.workspaceWriteTouches.length > 0
      && input.workspaceWriteTouches.every((entry) => this.normalizeProjectPathPointer(entry).startsWith('runs/'))
    if (
      finalStatus === 'success'
    ...
    if (finalStatus === 'no_delta' && blockedReason === 'missing_plan_deliverable_touch') {
      ...
      if (input.semanticGateConfig.enabled && input.semanticGateAudit.eligible) {
        ...
        const activeRules = this.parseDoneDefinitionRules(input.projectedPlanItem?.doneDefinition ?? [])
