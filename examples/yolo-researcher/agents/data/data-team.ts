 ...
 *       → execute Python (streaming) → collect outputs via manifest
 */
...
import type { DetailedTokenUsage } from '../../../../src/llm/provider.types.js'
import { aggregateUsage } from '../../../../src/llm/cost-calculator.js'
...
const SCHEMA_SCRIPT = join(MODULE_DIR, 'schema-inference.py')
...
export function createDataAnalyzer(config: {
  ...
  const llmClient = createLLMClientFromModelId(model, { apiKey })
  const tracker = config.tokenTracker
  const matplotlibQuickstart = loadMatplotlibQuickstart(projectPath)

  const outputBase = join(projectPath, '.yolo-researcher', 'outputs')
  const scriptsDir = join(projectPath, '.yolo-researcher', 'analysis', 'scripts')
  ...
  return {
    async analyze(input: AnalyzeInput): Promise<AnalyzeResult> {
      ...
      const runId = sessionId ? `run_${sessionId}_${Date.now()}` : `run_${Date.now()}`
      ...
      const usages: DetailedTokenUsage[] = []
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        tracker?.recordCall(model, result.usage)
...
${code}`
        ...
        if (!execResult.success) {
          ...
          return {
            ...
            usage: usages.length > 0 ? aggregateUsage(usages) : undefined
          }
        }
