 ...
 * - Full review saved to disk (.yolo-researcher/reviews/)
 *
...
import type { UsageSummary } from '../../../../src/llm/provider.types.js'
...
import { upsertPaperArtifact, updatePaperMetadata } from './paper-store.js'
...
export function createLiteratureTeam(config: {
  ...
  const createLLMRunner = (agent: SimpleAgent) => {
    return async (input: unknown) => {
      ...
      // Record usage from agent result into the shared tracker
      if (tracker && result.usage) {
        ...
        tracker.recordCall(model!, detailed)
      }
      ...
      if (result.success && result.finalState) {
        ...
        const searchResult: LiteratureSearchResult = {
          ...
          data: {
            ...
            llmCallCount: tracker?.getCallCount() ?? result.steps,
        ...
        return {
          ...
          localPapersUsed,
          externalPapersUsed,
          usageSummary: tracker?.getSummary()
        }
      }
