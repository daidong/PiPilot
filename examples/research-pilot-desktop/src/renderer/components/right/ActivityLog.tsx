...
export function ActivityLog() {
  ...
  return (
    <div>
      <h3 className="text-xs font-semibold t-text-muted uppercase tracking-wider mb-2 flex items-center gap-1.5">
        <Activity size={12} />
        Activity
      </h3>
  ...
  return (
    <div className="flex items-start gap-1.5 text-xs py-0.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        {event.error && (
          <p className="text-red-400 text-[10px] mt-0.5 truncate" title={event.error}>
            {event.error}
          </p>
        )}
      </div>
...
function eventColor(event: ActivityEvent) {
  ...
  if (event.type === 'system') return 'text-teal-400'
  if (event.type === 'tool-call') return 'text-teal-400'
