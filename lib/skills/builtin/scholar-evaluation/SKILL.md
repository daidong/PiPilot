...
- Call the **`generate_diagram`** tool. Load the `scientific-schematics` skill first if you want type-specific prompt guidance.
- Describe the desired diagram precisely; name components, labels, quantities, and flow direction.
...
```
generate_diagram(
  prompt: "<your diagram description>",
  output: "figures/<name>.png",
  doc_type: "journal" | "conference" | "thesis" | "report" | ...,
  diagram_type: "flowchart" | "pathway" | "architecture" | "circuit" | "network" | "conceptual"
)
```

The tool runs a verdict-driven generate → review loop, producing
publication-quality PNGs with a JSON review log beside each output.
...
Refer to `@skill/references/evaluation_framework.md` for detailed criteria and rubrics for each dimension.
...
To calculate aggregate scores programmatically, use `@skill/scripts/calculate_scores.py`.
...
### `@skill/references/evaluation_framework.md`
...
### @skill/scripts/calculate_scores.py
...
Usage:
```bash
python @skill/scripts/calculate_scores.py --scores <dimension_scores.json> --output <report.txt>
```
...
2. Load `@skill/references/evaluation_framework.md` for detailed criteria
