import { type CLIContext, type Literature, type Provenance } from '../types.js'
...
/**
 * Compose the provenance record for a newly created paper artifact.
 *
 * Precedence:
 *   1. Explicit `override` from the caller (e.g. importer passes
 *      `{ source: 'import', extractedFrom: 'file-import' }`) — wins per-field.
 *   2. Otherwise: agent-default when an `externalSource` is set (literature
 *      search and similar), else user-default.
 *
 * `sessionId` is always taken from the CLI context — it's a runtime fact,
 * not a caller decision.
 */
function buildProvenanceForCreate(
  context: CLIContext,
...
export function upsertPaperArtifact(
  ...
  opts: {
    ...
    /**
     * Caller-supplied provenance override. When provided, it wholly replaces
     * the auto-derived provenance below (the rule that defaults to
     * `source: 'agent'` whenever `externalSource` is set). Importers MUST
     * supply this to mark `source: 'import'` / `extractedFrom: 'file-import'`
     * — otherwise the BibTeX-imported papers would be mis-labelled as agent
     * products. RFC-006 review comment #1.
     *
     * `sessionId` is always taken from `context`. Other fields are taken
     * from this override when present, otherwise from the auto-derivation.
     */
  ...
  const created = artifactCreate({
    ...
    provenance: buildProvenanceForCreate(context, opts.externalSource, opts.provenance)
  }, context)
...
export function updatePaperMetadata(
  ...
  if ((!existing.bibtex || existing.bibtex.trim().length === 0) && enriched.bibtex) {
    update.bibtex = enriched.bibtex
    // Enrichment-supplied bibtex (CrossRef/Semantic Scholar) is not our
    // local fallback — flip the auto flag so future imports respect it.
    update.bibtexIsAutoGenerated = false
  }
