  ...
  return {
    ...
    execute: async (_toolCallId, rawParams) => {
      ...
      if (type === 'note') {
        ...
        // Track whether the caller supplied a curated bibtex (vs. our
        // fallback). Importers and downstream merge logic key off
        // `bibtexIsAutoGenerated` to decide whether they may overwrite the
        // field — RFC-006 review comment #2.
        const callerProvidedBibtex = typeof args.bibtex === 'string' && args.bibtex.trim().length > 0
        const bibtex = callerProvidedBibtex
          ? (args.bibtex as string)
          : `@article{${citeKey},\n  title = {${title}}\n}`
...
export function createArtifactUpdateTool(
  ...
  return {
    ...
    execute: async (_toolCallId, rawParams) => {
      ...
      // Build the patch by only including fields the caller actually sent.
      // `updateArtifact` shallow-merges via `{ ...existing, ...patch }`, so
      // any explicit `undefined` in the patch would clobber the existing
      // value (JS spread copies undefined). RFC-006 review comment #2 plus
      // the spread-undefined trap surfaced during testing.
      ...
      if (typeof args.abstract === 'string') patch.abstract = args.abstract
      if (typeof args.year === 'number') patch.year = args.year
      if (typeof args.venue === 'string') patch.venue = args.venue
      if (typeof args.url === 'string') patch.url = args.url
      if (typeof args.doi === 'string') patch.doi = args.doi
      if (typeof args.pdfUrl === 'string') patch.pdfUrl = args.pdfUrl

      // bibtex requires special handling: when the caller provides a curated
      // non-empty bibtex, also clear `bibtexIsAutoGenerated` so future
      // BibTeX imports know the field is curated and refuse to overwrite it.
      if (typeof args.bibtex === 'string' && (args.bibtex as string).trim().length > 0) {
        patch.bibtex = args.bibtex as string
        patch.bibtexIsAutoGenerated = false
      }

      const updated = updateArtifact(projectPath, id, patch, getTurnId?.())
