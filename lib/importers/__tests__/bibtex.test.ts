...
/**
 * Tear down a tmp project directory with retry/backoff.
 *
 * `upsertPaperArtifact` fires `writeArtifactLedgerCreate(...)` as
 * fire-and-forget (`void` prefix in memory-v2/store.ts), so when the
 * test's `finally` block hits, an async ledger append may still be
 * flushing into the project. A bare `rmSync` then races the write and
 * fails on macOS with `ENOTEMPTY` — observed flakily in CI. Node's
 * built-in `maxRetries` + `retryDelay` is enough here.
 */
