...
- `pipilot.chat.input_delta` (on `invoke_agent step` spans after the session's first step) — `{ body: { appended, removed, carriedOver } }`. The exact conversation-array diff between the current raw provider payload and the previous raw provider payload, computed before redaction/size-cap/blob spill. The extractor recognizes provider-specific conversation keys: `messages` (Anthropic, OpenAI-compatible chat, Mistral, Bedrock), `input` (OpenAI/Azure/Codex Responses), and `contents` (Google Gemini / Vertex). Non-array fields with those names are ignored rather than treated as conversation history.
